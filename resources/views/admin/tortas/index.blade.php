                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center; padding: 20px;">
                                No hay productos disponibles. <a href="{{ route('admin.tortas.create') }}">Crear uno nuevo</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <script>
        ...
        document.querySelectorAll('.deleteBtn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.getElementById('deleteItemInfo').textContent = `Producto: "${nombre}" será eliminado permanentemente.`;
                const baseRoute = '{{ route("admin.tortas.destroy", ":id") }}';
                document.getElementById('deleteForm').action = baseRoute.replace(':id', id);
