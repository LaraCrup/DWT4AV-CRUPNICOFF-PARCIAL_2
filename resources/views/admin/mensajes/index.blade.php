...
@section('pageTitle', 'Mensajes')

@section('content')
    <section>
        <div class="sectionHeader">
            <h1 class="fontTitle">Mensajes</h1>
        </div>
            ...
            <table class="fontBody">
                ...
                <tbody>
                    @forelse($mensajes as $mensaje)
                        <tr>
                            <td>{{ $mensaje->id }}</td>
                            <td>{{ $mensaje->usuario->name ?? 'Usuario desconocido' }}</td>
                            <td>{{ $mensaje->titulo }}</td>
                            <td>{{ $mensaje->created_at->format('d/m/Y H:i') }}</td>
                            <td>
                                <div>
                                    <a href="{{ route('admin.mensajes.show', $mensaje->id) }}" class="btn btnSee">
